...
const [formData, setFormData] = useState({
  ...
  const columns = [
    ...
    {
      name: <div className="text-blue-500 font-bold text-center text-lg w-full">Tên phương thức</div>,
      selector: row => row.name,
      // width: "15rem",
      cell: row => <div className="flex justify-center items-center text-center w-full">{row.name}</div>,
    },
    {
      name: <div className="text-blue-500 font-bold text-center text-lg w-full">Mã phương thức</div>,
  ...
  const getAllPayments = async () => {
    try {
      const response = await fetch(`${urladmin}/payment-method/all?size=10&page=${page}&search=${searchValue}`, {
        headers: headers
      });
...
const handleEditSubmit = async () => {
  const { id, name, code, status } = selectedPayments;
  ...
  try {
    const response = await fetch(`${urladmin}/payment-method`, {
      method: 'PATCH', 
      headers: headers,
...
// ]
  ...
  const renderModalContent = () => {
    switch (modalType) {
      case 'edit':
        return (
          <div className="p-6 bg-white rounded-lg">
            <h2 className="text-2xl font-bold mb-4">Chỉnh sửa phương thức thanh toán</h2>
            <form onSubmit={handleEditSubmit}>
              <div className="mb-4">
                <label className="block text-gray-700">Tên phương thức thanh toán</label>
                <input
                  type="text"
                  name="name"
                  value={selectedPayments.name}
              ...
              <div className="mb-4">
                <label className="block text-gray-700">Mã phương thức thanh toán</label>
                <input
                  type="text"
                  name="code"
                  value={selectedPayments.code}
                  onChange={handleInputChange}
                  className="w-full px-4 py-2 border rounded-lg bg-white outline-none"
                  required
                />
      ...
      case 'add':
        return (
          <div className="p-6 bg-white rounded-lg">
            <h2 className="text-2xl font-bold mb-4">Thêm mới phương thức thanh toán</h2>
  ...
  return (
    ...
    <Breadcrumbs aria-label="breadcrumb">
      ...
      <Link color="inherit" href="payments">
